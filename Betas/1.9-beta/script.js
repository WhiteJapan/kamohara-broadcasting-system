const DB = {
    ...
    HI: [
        ...
        { name: "名取", next: "060", soon: "061", canBeTerm: true, terminalNext: "062", terminalSoon: "063" },
...
// --- 設定管理 (Core Logic) ---
const SETTINGS_KEY = 'krab_settings_v1.9';
...
function updateSliderFill(el) {
    ...
    el.style.background = `linear-gradient(to right, var(--accent-secondary) 0%, var(--accent-secondary) ${percent}%, rgba(255, 255, 255, 0.08) ${percent}%, rgba(255, 255, 255, 0.08) 100%)`;
}
...
function updateHistoryUI() {
    ...
    container.innerHTML = settings.history.map(item => `
        <div class="history-item">
            <span class="history-txt">${item.txt}</span>
